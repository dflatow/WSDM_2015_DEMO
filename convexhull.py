...
def convexHull(P_):
    ...
    P = prep_coordinates_for_convex_hull(P_)
...
def reproject(latitude, longitude):
    ...
    x = [lon * lon_dist for lon in longitude]
...
def prep_coordinates_for_convex_hull(coordinates):

    unique_coordinates = [(x[0], x[1]) for x in coordinates]
    unique_coordinates = list(set(unique_coordinates))
    lats = [x[0] for x in unique_coordinates]
    lons = [x[1] for x in unique_coordinates]

    return zip(lats, lons)

def area_covered_by_points(coordinates):
    # convert nparrays to list of lists

    if len(coordinates) <= 2:
        return 0.0

    # returns area (km^2) of convex hull covering lats, lons
    c = convexHull(coordinates)
    lats_c = [x[0] for x in c]
    lons_c = [x[1] for x in c]
    lats_c, lons_c = reproject(lats_c, lons_c)
    
    return area_of_polygon(lats_c, lons_c) / 1000000.0
...
if __name__ == '__main__':
    ...
    lats = [40.0, 40.0, 41.0, 41.0, 41.0, 41.00]
    lons = [70.0, 71.0, 70.0, 71.0, 71.0, 71.00]
    coordinates = zip(lats, lons)
    print area_covered_by_points(coordinates)
